"""
AGV - SISTEMA DE GUIADO v6.0 PRO
==================================

Aplicación de escritorio (OpenCV + YOLOv8) que detecta un AGV (vehículo
guiado autónomo) en video en vivo, lo ubica dentro de un mapa de zonas de
almacén, calcula la ruta hacia un destino y muestra instrucciones de
navegación (GIRA IZQUIERDA / SIGUE DERECHO / etc.) sobre una interfaz con
sidebar interactivo dibujado a mano con primitivas de OpenCV (no hay Qt/Tk).

Para una guía completa de arquitectura, flujos y archivos de datos ver
``CORPUS.md`` (en este mismo directorio o en la carpeta empaquetada
``SISTEMA_GUIADO_AGV/``).

Resumen rápido de secciones del archivo (buscar los comentarios "# N."):
  1. Configuración global y rutas de archivos de datos
  2. Variables globales de estado (sistema, AGV, cámara, FPS)
  3. Colores de zonas
  4. Funciones de historial de rutas (rastro dibujado del AGV)
  5. Sidebar UI (pestañas NAVEGAR | RUTAS | CONFIG) + notificaciones
  6. Carga/guardado de calibración (zonas, caminos, ROI, rutas)
  7. Funciones de navegación (brújula, instrucciones por waypoint)
  8. Panel de diagnóstico (overlay lateral con estado del AGV)
  9. Modo CALIBRAR (clic de 4 esquinas por zona, en modo INDI)
 10. Modo MAPA (definir el polígono del área de pista)
 11. Modo AJUSTAR RUTAS (dibujar rutas personalizadas punto a punto)
 12. Procesamiento de frame principal — modo INDI (cámara real) y
     modo MULTI (vista en perspectiva/plano calibrado)
 13. Hilo de comandos por terminal (alternativa a la GUI)
 14. Callback unificado de mouse (sidebar + área de video)
 15. main() — bucle de captura, render y eventos de teclado

Modos de operación (estado_sistema['modo_operacion']):
  - INDI:  usa la cámara en vivo tal cual, zonas calibradas a mano.
  - MULTI: aplica una transformación de perspectiva (homografía) para
           ver el almacén "desde arriba" con un mapa de zonas fijo.

Requiere: ultralytics (YOLO), opencv-python, numpy. Ver requirements.txt.
"""
...
SIDEBAR_W = 270
CANVAS_H  = 600
_RUTAS_CONTENT_Y = 82    # y donde empieza el contenido de la pestaña (tras cabecera+tabs)
_RUTAS_VIRTUAL_H = 2000  # altura máxima del canvas virtual para la lista de rutas
_btn_rects_cache = {}
...
rutas_scroll_y    = 0           # desplazamiento vertical de la lista de rutas (px)
rutas_total_h     = 0           # altura total del contenido de la lista de rutas
...
def dibujar_sidebar(h, W=SIDEBAR_W):
    ...
    if tab_activa == 'NAVEGAR':
        ...
        if rutas_sub == 'LISTA':
            _tab_rutas_scroll(panel,btn_rects,y,h,W)
        else:
...
def _dibujar_scrollbar(panel, y_start, h, content_h, W=SIDEBAR_W):
    """Scrollbar vertical en el borde derecho del sidebar para la lista de rutas."""
    if rutas_total_h <= content_h:
        return
    sb_x  = W - 5
    sb_w  = 4
    sb_y1 = y_start
    sb_y2 = h
    sb_total = sb_y2 - sb_y1
    cv2.rectangle(panel, (sb_x, sb_y1), (sb_x + sb_w, sb_y2), (30, 35, 50), -1)
    visible_ratio = content_h / max(1, rutas_total_h)
    thumb_h  = max(20, int(sb_total * visible_ratio))
    max_scroll = rutas_total_h - content_h
    scroll_ratio = rutas_scroll_y / max(1, max_scroll)
    thumb_y = sb_y1 + int((sb_total - thumb_h) * scroll_ratio)
    cv2.rectangle(panel, (sb_x, thumb_y), (sb_x + sb_w, thumb_y + thumb_h), C_ACC, -1)


def _tab_rutas_scroll(panel, btn_rects, y_start, h, W=SIDEBAR_W):
    """Renderiza la lista de rutas con scroll virtual."""
    global rutas_total_h, rutas_scroll_y
    content_h = h - y_start

    v_panel = np.zeros((_RUTAS_VIRTUAL_H, W, 3), dtype=np.uint8)
    v_panel[:] = C_BG
    v_btn = {}
    content_end = _tab_rutas(v_panel, v_btn, 0, W)
    rutas_total_h = max(content_end, content_h)

    max_scroll = max(0, rutas_total_h - content_h)
    rutas_scroll_y = max(0, min(rutas_scroll_y, max_scroll))

    src_y1 = rutas_scroll_y
    src_y2 = min(rutas_scroll_y + content_h, _RUTAS_VIRTUAL_H)
    actual_h = src_y2 - src_y1
    if actual_h > 0:
        panel[y_start:y_start + actual_h, :] = v_panel[src_y1:src_y2, :]

    for bid, (bx1, by1, bx2, by2) in v_btn.items():
        adj_y1 = by1 - rutas_scroll_y + y_start
        adj_y2 = by2 - rutas_scroll_y + y_start
        if adj_y2 > y_start and adj_y1 < h:
            btn_rects[bid] = (bx1, adj_y1, bx2, adj_y2)

    _dibujar_scrollbar(panel, y_start, h, content_h, W)
...
def procesar_click_sidebar(sx, sy):
    ...
    global puntos_ruta_dibujada, ruta_seleccionada, rutas_scroll_y
...
def clic_universal(event, x, y, flags, param):
    ...
    global rutas_scroll_y
    if x >= VIDEO_W:
        if event == cv2.EVENT_LBUTTONDOWN:
            procesar_click_sidebar(x - VIDEO_W, y)
        elif event == cv2.EVENT_MOUSEWHEEL and tab_activa == 'RUTAS' and rutas_sub == 'LISTA':
            content_h = CANVAS_H - _RUTAS_CONTENT_Y
            delta = -40 if flags > 0 else 40
            max_scroll = max(0, rutas_total_h - content_h)
            rutas_scroll_y = max(0, min(max_scroll, rutas_scroll_y + delta))
        return
    # Área de video
    modo    = estado_sistema['modo']
